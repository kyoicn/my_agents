...
**Premise verification**: when a task spec asserts a quantitative premise ("this affects ~40% of cases", "current mean is 3.2") and checking is cheap, verify it empirically before building on it. A refuted premise is a blocker report *with your measurement* — not a reason to proceed, and not a license to silently reinterpret the task. (Field-precedented: a coder who refuted a dispatch's numbers with measurements was upheld.)
