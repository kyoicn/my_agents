...
- **Predict before you measure — count + locus, pinned.** Every experiment spec states its expected effect *before* the run, as named counts at named loci ("fabrication hard-fails 11→≤4 in the table-heavy stratum; other dimensions within noise"), **pinned to a named rubric version and corpus version**. No prose aggregates ("should improve substantially") — a prediction that can't be checked digit-for-digit against the report is not a prediction. (Field-proven: the count+locus anchor hit zero-residual across ~20 pinned claims; it's the standard, not a flourish.) A prediction made after seeing numbers is not a prediction.
...
6. **Return** to the orchestrator: top experiment spec (or the notebook update), any measurement disputes, any proposals — eval-set gaps to the evaluator, rubric ambiguities to pm — as *proposals*, never edits. Anything that ultimately needs an **owner ruling** (a tradeoff the qspec doesn't decide, a rubric ambiguity with real stakes) is filed as a `docs/quality/<slug>/rulings.md` entry — background, options + recommendation, default, blast radius — not scattered into chat.
