...
- **Quality surfaces are eval-triggering**: if a task touches files named in a `docs/quality/*/qspec.md` Surface section (prompt templates, pipeline modules), its "Done when" must include the smoke check (`scripts/eval-runner.sh <slug> ... smoke` shows no regression beyond the noise floor) — the cheap insurance against an unrelated refactor silently ruining output quality. But *improving* graded quality is never yours to decompose: that's `/quality-cycle`'s hypothesis loop (researcher designs experiments there, not you). If pm-review or the backlog asks for "better summaries," the task you write is "run /quality-cycle <slug>", not a decomposition of quality work.
